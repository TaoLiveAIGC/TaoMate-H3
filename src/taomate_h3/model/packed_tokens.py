...
"""Native MiniMax H3 packed-token transforms."""
...
def minimax_h3_patchify_video_latent(
    ...
    *,
    ...
    """Pack a video latent [B,C,T,H,W] into MiniMax H3 DiT token rows."""
...
def minimax_h3_unpatchify_video_tokens(
    ...
    *,
    ...
    """Unpack MiniMax H3 DiT rows into video latent [B,C,T,H,W]."""
...
def minimax_h3_unpack_audio_tokens(
    ...
    *,
    ...
    """Unpack channel-major DiT audio rows into [C,latent_dim,T]."""
    ...
    if audio_t <= 0 or audio_channel <= 0:
        raise ValueError(
            f"audio_t and audio_channel must be positive, got {audio_t=} {audio_channel=}"
        )
    if int(rows.shape[0]) != audio_t:
        raise ValueError(f"audio rows {int(rows.shape[0])} != audio_t {audio_t}")
    if audio_t % audio_channel:
        raise ValueError(
            f"audio_t must be divisible by audio_channel, got {audio_t=} {audio_channel=}"
        )
